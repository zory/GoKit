...
public class PathTweenGUI : BaseDemoGUI
{
    ...
    void Start()
    {
        ...
        if (LoadScriptableObject)
        {
            Debug.Log($"Loading path from Scriptable Object. {Path.Nodes.Count} nodes found!");
            path = new GoSpline(Path.Nodes);
        }
        else
        {
#if !UNITY_WEBPLAYER
            path = new GoSpline("demoRoute");
#else
// alternatively, paths can be created from an array of Vector3s
		var vectors = new Vector3[] { new Vector3( 0, 1, 1 ), new Vector3( 4, 5, 6 ) };
		path  = new GoSpline( vectors );
#endif
        }
